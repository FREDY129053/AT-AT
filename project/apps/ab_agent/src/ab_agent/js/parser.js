const parse = () => {
  ...
  // Создание уникальных имен(button -> button1 -> ... -> buttonk)
  ...
  // Сворчаивание сложных пустых div цепочек в одну
  const flatten = (el) => {
  ...
  function automaticStripElement(
    ...
    if (
      ...
      if (!inputIsDisabled && thisName) {
        ...
        clone.setAttribute(
          "parser-can-edit",
          !original.readOnly ? "true" : "false",
        );
        original.setAttribute("parser-semantic-id", thisName);
      }
      if (!inputIsDisabled && thisName && t === "number") {
        clone.setAttribute(
          "parser-numeric-value",
          original.valueAsNumber || "",
        );
      }
      if (
        !inputIsDisabled &&
        thisName &&
        original.selectionStart !== undefined
      ) {
  ...
  return {
    ...
    input_elements: Array.from(
      ...
      disabled: el.hasAttribute("parser-input-disabled") === true,
      type:
